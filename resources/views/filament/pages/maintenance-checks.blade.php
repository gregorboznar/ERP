        ...
        <div id="tab-{{ $machine->id }}" class="space-y-4 {{ !$loop->first ? 'hidden' : '' }}">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="p-4 border rounded-lg">
              ...
              <p class="text-gray-600 dark:text-gray-300">View and manage recent maintenance checks for {{ $machine->title }}</p>
            </div>

            <div class="p-4 border rounded-lg">
              <h3 class="text-lg font-semibold mb-2">Scheduled Maintenance</h3>
              <p class="text-gray-600 dark:text-gray-300">Upcoming scheduled maintenance tasks for {{ $machine->title }}</p>
            </div>

            <div class="p-4 border rounded-lg">
              <h3 class="text-lg font-semibold mb-2">Maintenance History</h3>
              <p class="text-gray-600 dark:text-gray-300">Historical maintenance records for {{ $machine->title }}</p>
            </div>
          </div>
        </div>
        @endforeach
